...
class SumNode(Node):
    ...
    def evaluate(self, spn):
        ...
        for child_id in self.children.keys():
            ...
            # forward pass ignore if weight is 0
            if self.children[child_id][0] == torch.tensor(0.0).cuda():
                continue

            # taking the log of the weight + log val
            child_val = spn.get_node_by_id(child_id).evaluate(spn)
            log_weight = torch.log(self.children[child_id][0])
            weighted_child = torch.add(child_val, log_weight)

            if weighted_child == Node.LOG_ZERO:
                continue

            weighted_children = Node.log_sum_exp(weighted_children, weighted_child)
