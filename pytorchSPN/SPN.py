...
class SPN():
    ...
    LOG_ZERO = torch.tensor(float('-inf'))
    if torch.cuda.is_available():
        LOG_ZERO = torch.tensor(float('-inf')).cuda()
    ...
    def normalise_weights(self):
        for node in self.get_sum_nodes():
            ...
            for child_id in node.children.keys():
                z = torch.add(z, node.children[child_id][0].data.cuda())
            if z == torch.tensor(0.0).cuda():
                return
            for child_id in node.children.keys():
                node.children[child_id][0].data = torch.div(node.children[child_id][0].data.cuda(), z)
